...
import { LOOP_SCHEDULER_CHANNEL_NAME, } from "./contract.js";
...
function getNextCronTime(cron, from = new Date()) {
    ...
    for (let attempts = 0; attempts < 525600; attempts++) {
        ...
        const dayOk = dayF === "*" ? dowMatch : dowF === "*" ? dayMatch : dayMatch || dowMatch;
        if (!dayOk) {
...
// ── settings 读写 ─────────────────────────────────────────────────
function readLoopsFromSettings(pi) {
    const settings = pi.getSettings();
    const ls = settings.loopScheduler;
    return ls?.loops ?? [];
}
async function persistLoops(_pi, _loops) {
...
// ── 主扩展 ────────────────────────────────────────────────────────
export default function loopSchedulerExtension(pi) {
    ...
    if (isScheduler) {
        lockHeartbeat = setInterval(() => {
            try {
                writeFileSync(getLockPath(), JSON.stringify({ pid: process.pid, ts: Date.now() }));
            }
            catch { /* */ }
        }, 30_000);
    }
    ...
    try {
        ...
        channel = createTypedChannel(rawChannel).server;
        const safeEmit = (event, data) => {
            try {
                rawChannel.emit?.(event, data);
            }
            catch {
        ...
        channel.handle("list", () => ({ loops: Array.from(jobs.values()).map((j) => j.config), isScheduler }));
        channel.handle("getStatus", () => ({ type: "status", loops: buildStatus(jobs) }));
        ...
        channel.handle("update", ({ id, ...updates }) => {
            ...
            if (!entry)
                return { ok: false, error: `Loop not found: ${id}` };
        ...
        const LoopManageParams = Type.Object({
            ...
            cron: Type.Optional(Type.String({ description: '标准 5-field cron，如 "*/5 * * * *" 或 "0 9 * * *"' })),
            ...
            deliverAs: Type.Optional(Type.Union([Type.Literal("followUp"), Type.Literal("steer")], { description: "followUp = 排队等待，steer = 插话打断" })),
        });
        pi.registerTool({
            name: "loop_manage",
            label: "Loop Manage",
            description: "管理定时任务（cron loop）。可以创建、查看、修改、开关、删除定时任务。" +
            ...
            execute: async (_toolCallId, params) => {
                switch (params.action) {
                    case "list": {
                        ...
                        return {
                            content: [{ type: "text", text: JSON.stringify({ loops }, null, 2) }],
                            details: { loops },
                        };
                    }
                    case "create": {
                        if (!params.name || !params.cron || !params.prompt) {
                            return {
                                content: [{ type: "text", text: "错误: create 需要 name, cron, prompt" }],
                                details: { ok: false },
                            };
                        }
                        if (!isValidCron(params.cron)) {
                            return {
                                content: [{ type: "text", text: `错误: 无效的 cron 表达式: ${params.cron}` }],
                        ...
                        emitStatus();
                        return {
                            content: [{ type: "text", text: `已创建定时任务「${loop.name}」(${loop.cron})，ID: ${loop.id}` }],
                            details: { ok: true, id: loop.id },
                        };
                    }
                    case "update": {
                        if (!params.id) {
                            return { content: [{ type: "text", text: "错误: update 需要 id" }], details: { ok: false } };
                        }
                        const entry = jobs.get(params.id);
                        if (!entry) {
                            return { content: [{ type: "text", text: `错误: 任务不存在: ${params.id}` }], details: { ok: false } };
                        }
                        if (params.name !== undefined)
                            entry.config.name = params.name;
                        if (params.cron !== undefined) {
                            if (!isValidCron(params.cron)) {
                                return { content: [{ type: "text", text: `错误: 无效 cron: ${params.cron}` }], details: { ok: false } };
                            }
                            entry.config.cron = params.cron;
                        }
                        if (params.prompt !== undefined)
                            entry.config.prompt = params.prompt;
                        if (params.deliverAs !== undefined)
                            entry.config.deliverAs = params.deliverAs;
                        if (entry.task) {
                            entry.task.cancel();
                            entry.task = null;
                        ...
                        emitStatus();
                        return {
                            content: [{ type: "text", text: `已更新任务「${entry.config.name}」` }],
                            details: { ok: true },
                        };
                    }
                    case "toggle": {
                        if (!params.id || params.enabled === undefined) {
                            return { content: [{ type: "text", text: "错误: toggle 需要 id 和 enabled" }], details: { ok: false } };
                        }
                        const entry = jobs.get(params.id);
                        if (!entry) {
                            return { content: [{ type: "text", text: `错误: 任务不存在: ${params.id}` }], details: { ok: false } };
                        ...
                        emitStatus();
                        return {
                            content: [{ type: "text", text: `任务「${entry.config.name}」已${params.enabled ? "启用" : "暂停"}` }],
                            details: { ok: true },
                        };
                    }
                    case "remove": {
                        if (!params.id) {
                            return { content: [{ type: "text", text: "错误: remove 需要 id" }], details: { ok: false } };
                        }
                        const entry = jobs.get(params.id);
                        if (!entry) {
                            return { content: [{ type: "text", text: `错误: 任务不存在: ${params.id}` }], details: { ok: false } };
                        ...
                        emitStatus();
                        return {
                            content: [{ type: "text", text: `已删除任务「${name}」` }],
                            details: { ok: true },
                        };
                    }
                    default:
                        return { content: [{ type: "text", text: `未知操作: ${params.action}` }], details: { ok: false } };
                }
            },
        });
        // ── /loops 命令 ──
        ...
        // ── context hook ──
        pi.on("context", (ctx, contextEntries) => {
            const activeLoops = Array.from(jobs.values()).filter((j) => j.config.enabled && j.task);
            if (activeLoops.length === 0)
                return;
            const summary = activeLoops
                .map((j) => `- ${j.config.name} (${j.config.cron}): 已运行 ${j.runCount} 次${j.lastRun ? `, 上次: ${new Date(j.lastRun).toLocaleString()}` : ""}`)
                .join("\n");
            contextEntries.push({
                role: "system",
...
// ── 启动单个 cron job ─────────────────────────────────────────────
function startJob(pi, jobs, id, emitStatus, schedulerActive) {
    const entry = jobs.get(id);
    ...
    const task = pi.background(async (signal) => {
