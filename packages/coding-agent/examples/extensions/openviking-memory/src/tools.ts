...
export function registerTools(pi: ExtensionAPI, config: OpenVikingConfig): void {
	...
	pi.registerTool(
		defineTool({
			...
			parameters: Type.Object({
				session_id: Type.String({ description: "Optional explicit OpenViking session ID. Omit to commit current session's mapped OV session." }),
			}),
			async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
				...
				if (!sessionId) {
					return { content: [{ type: "text", text: "Error: No OpenViking session associated. Start a normal session first or pass session_id." }], details: {}, isError: true }
				}
				...
				try {
					...
					if (resolvedMapping && piSessionId) await flushPendingMessages(piSessionId, resolvedMapping, config)
					...
					if (resolvedMapping?.commitInFlight && piSessionId) {
						...
						if (task?.status === "completed") {
							const n = totalMemoriesFromResult(task.result)
							return { content: [{ type: "text", text: JSON.stringify({ message: `Memory extraction complete: ${n} memories extracted`, session_id: task.result?.session_id ?? sessionId, status: task.status, memories_extracted: n, archived: task.result?.archived ?? false, task_id: task.task_id }, null, 2) }], details: {} }
						}
					}

					const tempMapping: import("./session-manager.js").SessionMapping = resolvedMapping ?? {
						ovSessionId: sessionId, createdAt: Date.now(), capturedMessages: new Set(), messageRoles: new Map(), pendingMessages: new Map(), sendingMessages: new Set(),
					}
					...
					if (commitStart.mode === "completed") {
						...
						return { content: [{ type: "text", text: JSON.stringify({ message: `Memory extraction complete: ${n} memories extracted`, session_id: commitStart.result.session_id ?? sessionId, status: "completed", memories_extracted: n, archived: commitStart.result.archived ?? false }, null, 2) }], details: {} }
					}

					const waitTask = resolvedMapping && piSessionId ? await waitForCommitCompletion(resolvedMapping, piSessionId, config, ctx.signal) : null
					if (!waitTask) return { content: [{ type: "text", text: JSON.stringify({ message: "Commit processing in background", session_id: sessionId, status: "accepted", task_id: commitStart.taskId }, null, 2) }], details: {} }
					const n = totalMemoriesFromResult(waitTask.result)
					return { content: [{ type: "text", text: JSON.stringify({ message: `Memory extraction complete: ${n} memories extracted`, session_id: waitTask.result?.session_id ?? sessionId, status: waitTask.status, memories_extracted: n, archived: waitTask.result?.archived ?? false, task_id: waitTask.task_id }, null, 2) }], details: {} }
				} catch (error: unknown) {
					return { content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}`, isError: true }], details: {} }
				}
			},
