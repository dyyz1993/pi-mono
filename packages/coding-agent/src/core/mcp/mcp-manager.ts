...
export class McpManager {
	...
	private callSemaphore: CallSemaphore | undefined;
	...
	async connectServer(name: string, config: McpServerConfig): Promise<void> {
		...
		if (existing?.client) {
			try {
				await (existing.client as Client).close();
			} catch {}
		}

		const entry: McpConnection = { name, config, status: "connecting", tools: [] };
	...
	private async doConnectWithTimeout(
		...
		timeoutMs: number = this.connectTimeoutMs,
	): Promise<void> {
	...
	private async doConnectServer(name: string, config: McpServerConfig, signal: AbortSignal): Promise<void> {
		...
		await client.connect(transport as McpTransport, { signal });
	...
	async refreshTools(serverName?: string): Promise<DiscoveredTool[]> {
		if (serverName) {
			...
			try {
				const { tools } = await (conn.client as Client).listTools();
	...
	async callTool(fullName: string, args: Record<string, unknown>, timeoutMs?: number): Promise<unknown> {
		...
		try {
			...
			try {
				return await (connection.client as Client).callTool(
	...
	async disconnectAll(): Promise<void> {
		...
		for (const connection of this.connections.values()) {
			try {
				if (connection.client) await (connection.client as Client).close();
