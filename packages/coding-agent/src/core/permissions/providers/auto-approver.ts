...
import { normalizePermissionPath } from "../path-patterns.ts";
import type { PermissionProvider } from "../provider.ts";
import { findDangerousCommandMatch } from "./dangerous-command.ts";
...
export interface AutoApproverProviderOptions {
	name?: string;
	priority?: number;
}

export function createAutoApproverProvider(options: AutoApproverProviderOptions = {}): PermissionProvider {
	return {
		name: options.name ?? "auto-approver",
		...
		check(ctx) {
			...
			if (READ_ONLY_TOOLS.has(toolName)) {
				...
				if (!rawPath) return { type: "allow", reason: `Auto-approved low-risk read tool "${ctx.toolName}".` };
				return decideFileToolPath({
					cwd: ctx.cwd,
					rawPath,
					toolName: ctx.toolName,
					workspaceReason: `Auto-approved workspace read tool "${ctx.toolName}".`,
				});
			}
			...
			if (WRITE_TOOLS.has(toolName)) {
				...
				return decideFileToolPath({
					...
					rawPath,
					toolName: ctx.toolName,
					workspaceReason: `Auto-approved workspace write tool "${ctx.toolName}".`,
				});
			}
...
function decideFileToolPath(input: { cwd: string; rawPath: string; toolName: string; workspaceReason: string }) {
	const normalizedPath = resolveAgainstCwd(input.cwd, input.rawPath);
	if (normalizedPath === input.cwd || normalizedPath.startsWith(`${input.cwd}/`)) {
		return { type: "allow" as const, reason: input.workspaceReason };
	}
	const pathDecision = decidePath(normalizedPath);
	if (pathDecision === "allow") {
		return { type: "allow" as const, reason: `Auto-approved user-writable path "${normalizedPath}".` };
	}
	if (pathDecision === "deny-protected") {
		return { type: "deny" as const, reason: "Autopilot blocked protected or sensitive path." };
	}
	return {
		type: "deny" as const,
...
type AutoDecision = "allow" | "deny-protected" | "deny-outside";
...
function decidePath(filePath: string): AutoDecision {
	if (isSensitivePath(filePath)) return "deny-protected";
	if (isKnownTemporaryPath(filePath)) return "allow";
	if (isProtectedPath(filePath)) return "deny-protected";
	if (isInside(filePath, normalizePermissionPath(homedir()))) return "allow";
	return "deny-outside";
}

function decideDangerousCommand(command: string): "allow" | "deny" | "pass" {
	...
	if (rmTargets.length > 0) {
		return rmTargets.every((target) => isKnownTemporaryPath(target)) ? "allow" : "deny";
	}
...
function isKnownTemporaryPath(filePath: string): boolean {
	return isInside(filePath, "/tmp") || isInside(filePath, "/private/tmp") || isInside(filePath, "/var/folders");
}
