  ...
  on_start:
    - type: command
      command: "date"`,
		...
		expect(agent.hooks?.tool_call?.[0]).toMatchObject({
			type: "command",
			command: "echo hi",
			if: "bash",
			async: true,
...
hooks:
  tool_call:
	...
	// Bug 4 fix: parseHooks must support http type, once/timeout fields, and HookGroup format
	it("parses http type hooks from agent frontmatter", () => {
		writeAgent(
			"http-hooks.md",
		...
		const [agent] = loadAgentsFromDir(testDir, "user");
		expect(agent.hooks).toBeDefined();
		const hook = agent.hooks?.tool_call?.[0];
		expect(hook).toBeDefined();
		if (hook && "type" in hook && hook.type === "http") {
			expect(hook.url).toBe("http://localhost:9999/hook");
			expect(hook.headers).toEqual({ "X-Custom": "test" });
			expect(hook.timeout).toBe(30);
		}
	});

	it("parses once and timeout fields on command hooks", () => {
		writeAgent(
			"once-hooks.md",
			`name: oh\ndescription: oh\nhooks:\n  tool_call:\n    - type: command\n      command: "echo hi"\n      once: true\n      timeout: 10`,
		);
		const [agent] = loadAgentsFromDir(testDir, "user");
		const hook = agent.hooks?.tool_call?.[0];
		if (hook && "type" in hook && hook.type === "command") {
			expect(hook.once).toBe(true);
			expect(hook.timeout).toBe(10);
		}
	});

	it("parses HookGroup format with matcher", () => {
		writeAgent(
			"group-hooks.md",
		...
		const [agent] = loadAgentsFromDir(testDir, "user");
		expect(agent.hooks?.tool_call).toHaveLength(1);
		const group = agent.hooks?.tool_call?.[0];
		if (group && "hooks" in group) {
			expect(group.matcher).toBe("Bash|Edit");
			expect(group.hooks).toHaveLength(2);
			const [cmdHook, promptHook] = group.hooks;
			if ("type" in cmdHook) {
				expect(cmdHook.type).toBe("command");
				expect(cmdHook.command).toBe("echo matched");
			}
			if ("type" in promptHook) {
