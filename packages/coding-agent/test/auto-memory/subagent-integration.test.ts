...
function createTestContext() {
	...
	const fakeChannel = { send: (data: unknown) => { events.push(data); } };
...
describe.skipIf(!hasModelConfig)("SubAgent E2E --model fallback", () => {
	it("no --model → falls back to global default (zhipuai/glm-4.7)", async () => {
		...
		try {
			const args = ["--mode", "json", "-p", "--session", sessionPath, "--no-extensions", "What is 10+20? Reply with just the number."];
	...
	it("explicit --model zhipuai/glm-4.7 with provider/model format", async () => {
		...
		try {
			const args = ["--mode", "json", "-p", "--session", sessionPath, "--no-extensions", "--model", "zhipuai/glm-4.7", "What is 2+3? Reply with just the number."];
