...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@dyyz1993");
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@dyyz1993+pi-coding-agent@0.67.68\\node_modules\\@dyyz1993\\pi-coding-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@dyyz1993/pi-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @dyyz1993/pi-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@dyyz1993/pi-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@dyyz1993/pi-coding-agent")).toBe(
			"Update @dyyz1993/pi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@dyyz1993/pi-coding-agent");
		...
		expect(command).toEqual({
			...
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @dyyz1993/pi-coding-agent`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@dyyz1993/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @dyyz1993/pi-coding-agent`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@dyyz1993\\pi-coding-agent";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@dyyz1993/pi-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @dyyz1993/pi-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@dyyz1993/pi-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @dyyz1993/pi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@dyyz1993/pi-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@dyyz1993", "pi-coding-agent");
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@dyyz1993/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@dyyz1993/pi-coding-agent")).toContain(
