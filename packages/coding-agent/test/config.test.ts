...
describe("detectInstallMethod", () => {
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "@dyyz1993/pi-coding-agent"],
			display: `npm --prefix ${prefix} install -g @dyyz1993/pi-coding-agent`,
		});
	});
	...
	test("self-update treats empty npmCommand as unset", () => {
		...
		expect(command?.args).toEqual(["--prefix", prefix, "install", "-g", "@dyyz1993/pi-coding-agent"]);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(`npm --prefix "${prefix}" install -g @dyyz1993/pi-coding-agent`);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "@dyyz1993/pi-coding-agent"],
			display: "bun install -g @dyyz1993/pi-coding-agent",
		});
	});
