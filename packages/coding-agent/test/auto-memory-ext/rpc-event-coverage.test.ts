...
describe.skipIf(!hasApiKey)("auto-memory RPC event coverage", { sequential: true, timeout: 300_000 }, () => {
		...
		it("creates memory files and updates MEMORY.md index", async () => {
			...
			try {
				...
				if (!existsSync(join(memoryDir, ENTRYPOINT_NAME))) {
					const { updateMemoryIndex } = await import("../../extensions/auto-memory/index.js");
					mkdirSync(memoryDir, { recursive: true });
	...
	describe("Category I: Channel list", () => {
		it("responds to list channel message with file listing", async () => {
			...
			try {
				...
				const response = (await memoryChannel.call("memory.list", {}, 60_000)) as any;
