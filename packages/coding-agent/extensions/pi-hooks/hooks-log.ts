 ...
 *
 * Session-scoped: lives as long as the pi-hooks extension instance.
...
export interface HookConfigSnapshot {
	...
	/** Per-rule skip list. Rules matching (event + matcher) are skipped until unskipped. */
	skippedRules: { event: string; matcher: string }[];
