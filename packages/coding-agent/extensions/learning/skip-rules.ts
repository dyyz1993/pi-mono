/**
 * Skip Rules System for Learning Extension.
 *
 * Copied from legacy memory/skip-rules.ts with minimal changes:
 * - getGlobalMemoryDir() → getGlobalLearningDir() (uses ~/.pi/agent/learning)
 * - Logger prefix changes from [legacy memory] to [learning]
 * - All type exports preserved for compatibility
 */
...
export function getGlobalLearningDir(): string {
  return join(homedir(), ".pi", "agent", "learning");
}
...
export function loadSkipWordStore(dir: string): SkipWordStore {
  ...
  try {
    ...
    // Backward compat: if no excludeKeywords, use empty array
