...
export default function piGoalExtension(pi: ExtensionAPI): void {
	...
	pi.on("session_compact", (event, ctx) => {
		...
		// NOTE: upstream uses event.willRetry (0.82+); this fork exposes event.fromExtension.
		// Only trigger continuation when the compaction was NOT extension-initiated.
		if (state.status === "setting_up") {
			persist(ctx, "setup_session_compacted", "same-conversation setup state revalidated after compaction");
			if (!event.fromExtension && !state.setupAwaitingUser) triggerSetupConversation(state, "Continue goal setup after compaction without repeating resolved questions.");
			return;
		}
		persist(ctx, "session_compacted", "goal state revalidated after compaction");
		if (!event.fromExtension && canResumeAutonomy(ctx, state)) triggerContinuation(state, "Continue after compaction with refreshed durable goal context.");
	});
