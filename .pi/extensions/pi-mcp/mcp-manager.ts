...
export class McpManager {
  ...
  async connectAll(
    ...
    console.log(
      `[pi-mcp] ${succeeded} server(s) connected${failed > 0 ? `, ${failed} failed` : ""}`,
    );
  }
  ...
  private async connectServer(
    ...
    try {
      ...
      console.log(
        `[pi-mcp] "${name}": ${entry.tools.length} tool(s) discovered`,
      );
    } catch (e) {
      entry.status = "error";
      entry.error = e instanceof Error ? e.message : String(e);
      console.error(`[pi-mcp] "${name}" failed: ${entry.error}`);
    }
  }
  ...
  async callTool(
    ...
    if (!connection?.client) {
      throw new Error(`MCP server "${mapping.serverName}" not connected`);
    }
