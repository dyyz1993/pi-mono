/**
 * Lossless Memory - MiniMax LLM 评估示例
 * 
 * 运行方式：
 *   npx tsx example-minimax-evaluation.ts
 */

import { MemoryDatabase } from "./database.js";
import { DAGManager } from "./dag-manager.js";
import { MemorySystemEvaluator } from "./evaluator.js";
import { MiniMaxLLMEvaluator } from "./minimax-evaluator.js";
...
const TEST_DB = path.join(__dirname, "test-minimax-evaluation.db");
...
console.log("╔══════════════════════════════════════════════════════╗");
console.log("║   Lossless Memory - MiniMax LLM 评估系统              ║");
console.log("╚══════════════════════════════════════════════════════╝\n");

const MINIMAX_CONFIG = {
  apiKey: process.env.MINIMAX_API_KEY || "sk-cp-w4KKnnlDoUCIGX6bp1IXFfa8FKXPLK-3mrX0NX5m7WS6JjXN26R4Spk3h8fvEyOkN4vWpchAH3Sm1lXNA34KhX0KwnLZLNpYMsVziDOPCcwr02A0M0914ZA",
  baseUrl: process.env.MINIMAX_BASE_URL || "https://api.minimaxi.com/v1",
  model: process.env.MINIMAX_MODEL || "MiniMax-M2.5",
};

console.log(`MiniMax 配置:`);
console.log(`  API URL: ${MINIMAX_CONFIG.baseUrl}`);
console.log(`  模型: ${MINIMAX_CONFIG.model}`);
console.log(`  API Key: ${MINIMAX_CONFIG.apiKey.slice(0, 20)}...\n`);
...
dag.initializeForSession("minimax-evaluation-session");
...
for (const conv of conversations) {
  ...
  for (let i = 0; i < conv.messages.length; i++) {
    const messageNode = dag.createNode({
      type: "original",
      level: 0,
      content: conv.messages[i].content,
      childIds: [],
      sessionEntryIds: [],
      tokenCount: conv.messages[i].content.length,
    });
    messageNodeIds.push(messageNode.id);
  }
  
  allEntryIds.push(...messageNodeIds);
  ...
  const l1Node = dag.createNode({
    ...
    sessionEntryIds: messageNodeIds,
    tokenCount: 100 + Math.floor(Math.random() * 50),
  });
  
  // 设置消息节点的父节点
  for (const msgId of messageNodeIds) {
    const msgNode = dag.getNode(msgId);
    if (msgNode) {
      msgNode.parentIds = [l1Node.id];
      db.updateNode(msgNode);
    }
  }
  ...
  console.log(`    ✅ 创建 L1 节点: ${l1Node.id.slice(0, 8)}... (${messageNodeIds.length} 个消息节点)`);
}
...
console.log("══════════════════════════════════════════════════════");
console.log("🤖 步骤 4: 运行 MiniMax LLM 评估");
console.log("══════════════════════════════════════════════════════\n");

console.log("  ⏳ 正在调用 MiniMax API 进行深度评估...\n");

try {
  const minimaxEvaluator = new MiniMaxLLMEvaluator(db, dag, CONFIG, MINIMAX_CONFIG);
  const llmResult = await minimaxEvaluator.runLLMEvaluation();

  minimaxEvaluator.printLLMReport(llmResult);
  ...
  console.log(`MiniMax 质量得分: ${(llmQualityScore * 100).toFixed(2)}/100`);
  console.log(`MiniMax 保持率得分: ${(llmRetentionScore * 100).toFixed(2)}/100`);
  ...
  console.error("❌ MiniMax LLM 评估失败:", error);
  console.log("\n可能的原因:");
  console.log("  1. MiniMax API Key 无效");
...
console.log("提示:");
console.log("  - 设置 MINIMAX_API_KEY 环境变量来覆盖默认 API Key");
console.log("  - 设置 MINIMAX_BASE_URL 环境变量来覆盖默认 API URL");
console.log("  - 设置 MINIMAX_MODEL 环境变量来选择模型");
console.log("");
